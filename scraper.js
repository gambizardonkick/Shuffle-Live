...
async function fetchCryptoPrices() {
    try {
        ...
        // Rate limit or network error - use default prices to not block scraper
        if (error.response?.status === 429) {
            console.log('⚠️ Crypto price API rate limited - using fallback prices');
        } else {
            console.log('⚠️ Could not fetch crypto prices:', error.message);
        }
        // Set reasonable fallback prices if we don't have any yet
        if (cryptoPrices.BTC === 0) {
            cryptoPrices.BTC = 90000;
            cryptoPrices.ETH = 3400;
            cryptoPrices.SOL = 160;
...
async function startScraper(onBetFound) {
    ...
    try {
        ...
        // Railway/Render will use Puppeteer's downloaded Chrome automatically
        // No need to specify path - let Puppeteer find it
        ...
        await page.goto(SHUFFLE_URL, { waitUntil: 'networkidle0', timeout: 60000 });
        ...
        const pageStructure = await page.evaluate(() => {
            ...
            // Look for "Live Bets" text
            const allText = document.body.innerText;
            structure.liveBetsSection = allText.includes('Live Bets') || allText.includes('Activity');
        ...
        // Scroll to bottom to ensure live bets are loaded
        console.log('⬇️ Scrolling to load live bets...');
        await page.evaluate(() => {
        ...
        // Check tables again after scrolling
        console.log('🔍 Looking for Live Bets table after scroll...');
        ...
        console.log('📋 Tables after scroll:', JSON.stringify(tableInfo, null, 2));
        
        // Wait for and find the live bets table
        const liveBetsFound = tableInfo.some(t => t.columnCount >= 5);
        
        if (!liveBetsFound) {
            console.log('⚠️ Live bets table still not found - trying to wait for dynamic content...');
            await new Promise(resolve => setTimeout(resolve, 5000));
        } else {
            console.log('✅ Found Live Bets table with', tableInfo.find(t => t.columnCount >= 5).rowCount, 'rows!');
        }
